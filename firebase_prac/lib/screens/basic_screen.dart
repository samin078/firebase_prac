...
class _BasicScreenState extends State<BasicScreen> {
  List<String> currentUserSelectedCategories = [];
 ...
 // List<String> nearbyUsers = [];
  ...
  Future<void> fetchAndDisplayAllUsers() async {
    ...
    List<DocumentSnapshot> userDocuments = await databaseMethods.fetchAllUsers();
  ...
  Future<void> fetchUserLocation(String userId) async {
    try {
      ...
      if (userLocation != null) {
        ...
        print("Latitude: $currentUserLatitude, Longitude: $currentUserLongitude");
      } else {
  ...
  Future<List<String>?> getUserSelectedCategories(String userId) async {
  ...
  Map<String, List<String>> nearbyUsersCategories = {};
  Map<String, List<String>> nearbyUsersCommonCategories = {};

  Future<void> fetchNearbyUsers(double searchRadius) async {
    // Create a Firestore collection reference to your users
    CollectionReference usersCollection = FirebaseFirestore.instance.collection("user_info");
    ...
    List<String>? currentUserCategories = await getUserSelectedCategories(userId!);
    ...
    for (QueryDocumentSnapshot doc in querySnapshot.docs) {
      ...
      if (location != null && userId!=nuserId) {
        ...
        double distance = Geolocator.distanceBetween(
            currentUserLatitude, currentUserLongitude, userLatitude, userLongitude);
        print(distance);

        // If the distance is within the specified radius, add the user to the map
        if (distance <= searchRadius*5) {
          // Call the method to get the selected categories of the nearby user
          List<String>? nearbyUserCategories = await getUserSelectedCategories(nuserId);

          List<String> commonCategories = [];
          if (currentUserCategories != null && nearbyUserCategories != null) {
            commonCategories = currentUserCategories.toSet().intersection(nearbyUserCategories.toSet()).toList();
          }
          print(nuserId);

          // Update the UI with nearby user data
          setState(() {
            nearbyUsersCategories[nuserId] = nearbyUserCategories ?? [];
            nearbyUsersCommonCategories[nuserId] = commonCategories;
          });
        }
      }
    }
  ...
  Future<void> fetchAllUserLocation(double currentUserLatitude, double currentUserLongitude, String userId) async {
    DatabaseMethods databaseMethods = DatabaseMethods();
    List<DocumentSnapshot> userDocuments = await databaseMethods.fetchAllUsers();
    ...
    return Scaffold(
      ...
      body: Center(
        child: ListView.builder(
          itemCount: nearbyUsersCategories.length,
          itemBuilder: (context, index) {
            final userId = nearbyUsersCategories.keys.elementAt(index);
            final userCategories = nearbyUsersCategories[userId];
            final commonCategories = nearbyUsersCommonCategories[userId];
            ...
            return ListTile(
              ...
              subtitle: Column(
                ...
                children: [
                  Text('Categories: ${userCategories?.join(', ') ?? ''}'),
                  Text('Common Categories: ${commonCategories?.join(', ') ?? ''}'),
                ],
              ),
            );
          },
